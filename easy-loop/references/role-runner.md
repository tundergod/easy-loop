...
2. Spawn a fresh subagent per phase — planner (only when the contract needs revision), generator, then evaluator — injecting the file contents and envelope fields per the protocol's Injection table (resolve files via the envelope's `skill_path`). Set each subagent's model per the envelope's `model_policy` (off → no override, so it inherits the session model). If the contract is **staged** (`Staged: yes` in its `## Acceptance Checklist`), set the generator envelope's `milestone` field to the next milestone whose items are not all passing per the best kept iteration's `eval.json` (the first milestone on iteration 1), so it advances one per round rather than sprawling across the whole contract.
