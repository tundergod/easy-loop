...
1. Read `state.json`; if `status` is `cancelled`, stop immediately without writing. Otherwise create `iterations/<NNNN>/` and copy the contract's allowed-path files into `iterations/<NNNN>/snapshot/` (the revert substrate).
