...
- Model tiers default off (subagents inherit the session model); if the user wants to control cost, turn them on and record per-role tiers in the contract's `## Models` (`references/loop-protocol.md`).
